...
def _backbone_indices(n, reserve_ratio, seed=42):
    """Return indices for the backbone-only portion of a dataset of size n.

    Must match the split logic in changeagent_dpo/data.py so that backbone
    and DPO stages see perfectly complementary subsets.
    """
    g = torch.Generator()
    g.manual_seed(seed)
    perm = torch.randperm(n, generator=g).tolist()
    n_dpo = max(1, int(n * reserve_ratio))
    return sorted(perm[n_dpo:])  # backbone gets the complement
...
def create_data_loaders(args, train_transform, val_transform):
    ...
    if args.dpo_reserve_ratio > 0.0:
        backbone_idx = _backbone_indices(len(train_data), args.dpo_reserve_ratio)
        train_data = torch.utils.data.Subset(train_data, backbone_idx)
        print(f"[split] backbone train size: {len(train_data)} "
              f"(reserved {args.dpo_reserve_ratio:.0%} for DPO)")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=0.0,
        type=float,
        help='Fraction of train set to reserve exclusively for DPO post-training. '
             'Backbone only trains on the complementary portion. '
             'Must match the value used in train_verifier.py and train_agent_dpo.py. '
             '0.0 = use all training data (original behaviour).'
    )
